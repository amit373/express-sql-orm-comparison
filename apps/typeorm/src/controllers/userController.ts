...
/**
 * Controller class for handling user-related operations in the TypeORM app.
 */
export class UserController {
  ...
  updateUserStatus = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { isActive } = req.body; // status vs isActive. User API says /status. Assuming body contains isActive or status.
    // Let's assume the body has { status: 'active' | 'inactive' } or { isActive: boolean }
    // The requirement says "Activate / Deactivate user".
    // I'll check if isActive is provided, or status.
    const active =
      isActive !== undefined ? isActive : req.body.status === "active";
    const user = await this.userService.updateUserStatus(Number(id), active);
    return res.json(
      successResponse(SUCCESS_MESSAGES.USER_UPDATED_SUCCESSFULLY, user),
    );
  });
